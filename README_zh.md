...
当您开启Hyper-V功能后，主机系统将变成属于根分区的一个具有特权的虚拟机。创建的虚拟机属于子分区，它们相互隔离，无法感知彼此的存在。
...
基于这样的事实，您可以将虚拟机看作一个个隔离的小房间，在里面运行具有潜在威胁的程序、测试系统功能、多开游戏或者其他用途，而不用担心弄糟主机系统（1.PCIe 直通中不支持 FLR 的情况除外，虚拟机重启会连带主机重启。2.具有横向渗透能力的病毒也不在此列，请注意网络安全）。
...
Classic 调度器的出现时间可以追溯到 Windows Server 2008，是基于传统时间片轮换的公平分配原则，将虚拟机的处理器时间随机分给主机所有可用的逻辑处理器；在主机资源空闲的情况下，更可能分配到不同物理核心的逻辑处理器，而不是超线程，从而获取更好的性能。

Core 调度器出现时间稍晚，自 Windows Server 2016 以及 Windows 10 Build 14393推出，目的在于缓解侧信道攻击，即使是主机资源空闲的情况下，也更倾向于分配同一个物理核心的两个线程，而不是更多物理核心。这样的策略有助于提高安全性和虚拟机隔离，但是会显著降低主机资源空闲时虚拟机能分配到的CPU性能。从 Windows Server 2019 起，Windows Server 将默认使用核心调度器。
...
系统类型与调度器的类型无关，可以任意切换，重启主机后生效。

---
### 处理器（vCPU）
> [!NOTE]
> 虚拟机向主机申请逻辑处理器执行时间的调度能力。
...
若所有虚拟机的核心总数超过主机物理逻辑核心数（超售），需要即时响应的应用会受到极大影响。
...
开启后对虚拟机通过 VMBus 通信的 I/O 请求进行监测，出现中断风暴等异常行为时降低 CPU 执行时间分配，以防止影响根分区的主机系统。普通用户无需开启。
...
开启后将屏蔽 CPU 的部分指令集，取决于 [cpu-migration-compatibility.md](doc/cpu-migration-compatibility.md)，便于在不同硬件的主机上进行实时迁移。普通用户无需开启。
...
开启后透传 CPU 的硬件计数器，允许虚拟机里的开发工具直接访问物理 CPU 的性能监测硬件，需主机支持。
...
开启后让虚拟机的插槽拓扑跟随主机的物理插槽来划分，便于虚拟机按真实插槽做 NUMA 调度。
...
手动指定虚拟机所见的 L3 路数。当集群中不同主机的 L3 几何不一样时，将虚拟机的 L3 路数统一为固定值，能让虚拟机里依赖缓存拓扑的软件在迁移前后看到一致的缓存信息。
...
解除主机对虚拟机的频率上限约束，需要 CPU 支持调频让渡能力。

##### 暴露 PMU / LBR / PEBS / IPT

将物理 CPU 的性能监控硬件透传给虚拟机，供虚拟机里的性能分析与调试工具直接使用，需主机支持。
...
虚拟机在开机时必须占用的物理内存量。若主机空闲内存不够，可能会启动失败。

若操作系统支持热调整，可在运行时修改此数值从而增大或缩减内存分配上限。

##### 内存权重

当主机物理内存不足时，多个虚拟机之间争抢内存的优先级。

##### 动态内存

允许 Hypervisor 根据虚拟机内部的实际需求，实时伸缩分配的内存量。

最小内存不能大于启动内存。虚拟机最多能获得的物理内存不会最大内存或主机物理内存的上限。
...
决定虚拟内存与物理内存映射时的“颗粒度”，可选4K、2M、1G。该选项需要主机系统版本大于26100，并且虚拟机配置版本大于12.0，有利于大型数据库或高性能计算任务。

##### 内存加密

开启后将利用硬件特性（AMD SEV 或 Intel TDX）对内存数据进行实时加密，即使是主机也无法读取内存数据，开启后会带来轻微的内存延迟和 CPU 负载增加。
...
##### Intel SGX 机密计算
利用 Intel SGX 在处理器内部划出硬件级隔离的安全飞地（Enclave），飞地内的代码和数据在运行时对主机完全不可见。
...
##### 内存优化
三个独立的内存性能优化开关。
- 内存活跃度提示：向 Hypervisor 上报虚拟机内部的冷热内存页信息，辅助主机更智能地进行内存回收与换页决策，可降低内存压力下的性能抖动。
- 半虚拟化分页优化：启用启发式页错误，由虚拟机主动通知 Hypervisor 缺页事件，减少不必要的拦截开销，提升内存密集型负载的吞吐量。
- 独立压缩内存池：为虚拟机分配专属的内存压缩存储区，主机内存紧张时优先压缩而非换页到磁盘，可缩短内存回收延迟。
##### NUMA 节点内存块限制
...
- 物理固定：启用 GPA Pinning，将虚拟机的物理地址空间锁定在主机内存中，禁止换页或迁移。适用于对内存延迟极度敏感或需要 DMA 稳定映射的场景。
---
### 存储
> [!NOTE]
> 虚拟机能访问的本地存储介质。

分为虚拟文件和物理设备。虚拟文件可选择vhdx、vhd和iso等格式。物理设备可选择主机上可脱机的硬盘或光驱进行直通，部分 USB 存储介质可能不支持。
...
### 显卡
> [!NOTE]
> 虚拟机通过 GPU-PV 技术访问主机物理显卡的能力。

GPU-PV 是一种半虚拟化技术，它允许多个虚拟机共享使用物理 GPU 的计算能力，而无需 PCIe 直通。GPU-PV 仍然在不断进化，WDDM 版本越新，功能越全。主机和虚拟机尽量使用最新的系统版本。
...
· 启用了 GPU-PV 的显卡必须存在于主机，不能同时用于 PCIe 直通。

· 从同一张显卡获取的多个 GPU-PV 显卡分区不能提供超过物理上限的算力。

· 虚拟机可以获取来自不同显卡的 GPU-PV 显卡分区。

· 可能存在[内存泄露问题](https://github.com/jamesstringer90/Easy-GPU-PV/issues/446)，建议将主机系统版本更新到 `26100.4946`以上。

#### WDDM 版本与 GPU-PV 功能
> WDDM (Windows Display Driver Model) 版本越高，GPU-PV 功能越完善。建议主机和虚拟机都使用最新的 Windows 版本。

| Windows 版本 (Build) | WDDM 版本 | 虚拟化相关功能更新 |
| :--- | :--- | :--- |
| 17134 | 2.4 | 首次引入GPU 半虚拟化技术。 |
| 17763 | 2.5 | 优化主机与虚拟机间的资源管理与通信。 |
...
| **Nvidia** | Tesla V100-SXM2-16GB | Volta | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | 启动会导致主机崩溃 #95 |
...
| **AMD** | Radeon 890M | RDNA 3.5 | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | 启动会导致主机崩溃 |
...
向主机系统环境添加注册表，禁用安全策略等避免分配 GPU-PV 后无法启动虚拟机。
...
探测主机物理寻址能力，自动配置 MMIO 空间，开启写合并缓存。
...
- 对于 Windows 虚拟机，将全量注入主机驱动文件夹到虚拟机指定分区。对于 NVIDIA，ExHyperV 只会创建从 `HostDriverStore` 加载当前所选 `nvlddmkm.sys` 所需的最小 `nvlddmkm` 服务值，不会导入主机完整的 NVIDIA 服务树。同时，会为虚拟机的 System32 目录创建部分驱动文件的链接，具体映射关系参考 [drivermapping.md](https://github.com/Justsenger/ExHyperV/blob/main/doc/drivermapping.md)。
...
#### 硬件加速
> [!NOTE]
> 将原本需要主机 CPU 处理的工作交给物理网卡硬件完成，从而提升性能。
...
PCIe 直通允许将一个完整的 PCIe 设备（显卡、网卡、声卡、USB 控制器等）从主机移除并直接分配给虚拟机。
...
1.  **主机态**: 设备正常挂载到主机系统，只能被主机使用。
2.  **卸除态**: 设备已从主机卸载 (`Dismount-VMHostAssignableDevice`)，但未分配给虚拟机。此时设备在主机设备管理器中不可用，需要重新挂载到主机或分配给虚拟机。
3.  **虚拟态**: 设备已成功分配给虚拟机。
...
- **功能层复位 (FLR)**: 若不支持，重启虚拟机会连带主机重启。
- **物理显示输出**: 虚拟机能否通过显卡的物理接口（HDMI/DP）输出画面。
---
### 虚拟交换机
> [!NOTE]
> 显示主机上 Hyper-V 交换机的拓扑结构以及连接状况。

ExHyperV 将 HyperV 的三种交换机类型（外部、内部、私有）重新定义为三种网络模式（桥接、NAT、无上游），其中 NAT 模式集成了 ICS 功能。

### 网络模式

桥接模式：主机和虚拟机将连接在同一个外部虚拟交换机下面，由指定物理网卡提供出口网络。

NAT 模式：主机和虚拟机将连接在同一个内部虚拟交换机下面，主机通过 ICS 将物理网卡的网络共享给虚拟机，并负责上游出口、NAT 转换以及 DHCP 。只能存在一个NAT模式的网络。

无上游：主机和虚拟机将连接在同一个内部虚拟交换机下面，没有上游网络。主机可以选择不连接到该交换机，此时虚拟交换机自动切换为私有交换机。
...
USB 直通允许将主机上的 USB 设备分配给虚拟机独占使用，无需 PCIe 直通，也不受 IOMMU 限制。
...
ExHyperV 采用 VMBus 中的 af-hyperv 协议包装 USBIP 协议，在主机与虚拟机之间建立高性能数据通道，无需任何网络配置。
...
- 目前仅维护 Windows 主机 → Windows 虚拟机链路；Windows → Linux 理论可行，可向 [ExHyperV-USBProxy](https://github.com/Justsenger/ExHyperV-USBProxy/releases/latest) 提交 PR。
...
![](https://img.shields.io/badge/传说-Ermo-24292e?style=for-the-badge&logo=starship&logoColor=BE64FF&labelColor=24292e&color=BE64FF)
